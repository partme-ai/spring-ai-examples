# Spring AI 入门实践：Spring AI 生成式人工智能堆栈（GenAI Stack）

> 基于 Spring AI 框架实现完整的 GenAI Stack 集成，结合 Neo4j 图数据库、Ollama 本地模型，构建端到端的 RAG 应用，支持知识图谱和向量搜索。
...
本项目展示如何使用 Spring AI 集成 GenAI Stack 的核心组件：Neo4j 图数据库、Ollama 本地模型运行时，构建强大的 RAG（检索增强生成）应用。
...
| Neo4j | 5.15+ | 图数据库与向量存储 |
| Ollama | Latest | 本地模型运行时 |

### 1.3 代码地址
**GitHub**：https://github.com/partme-ai/spring-ai-examples/tree/main/spring-ai-genai-stack
**本地路径**：`spring-ai-genai-stack/`
...
> ⚠️ 注：性能基准数据待补充。如需性能数据，请参考 [Neo4j 官方文档](https://neo4j.com/docs/) 或 [Ollama 官方文档](https://ollama.com/docs)。
...
public class GenAIClient {
    ...
    public String queryWithGraph(String question) {
        String url = BASE_URL + "/rag/query?question={question}";
        return restTemplate.getForObject(url, String.class, question);
    }

    public Map<String, Object> getGraphStats() {
        return restTemplate.getForObject(BASE_URL + "/graph/stats", Map.class);
    }
}
```
...
- **感谢 Neo4j 团队** 提供强大的图数据库和向量搜索能力
- **感谢 Ollama 团队** 提供简单易用的本地模型运行环境
- **感谢 Spring AI 团队** 提供完善的 AI 能力集成框架
- **感谢开源社区** 提供丰富的技术资源
