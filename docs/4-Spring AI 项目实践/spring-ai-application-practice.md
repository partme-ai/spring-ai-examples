...
核心类：`io.github.partmeai.aisql.controller.SqlController`
...
当前入口为 **`io.github.partmeai.Main`**，为 **占位演示**（打印循环），尚未接入 Spring AI；可作为后续 **酒店推荐等业务** 的聚合模块骨架。扩展时建议：引入 Spring Boot 启动类、领域服务与 `ChatClient`/`VectorStore` 等 Bean，并复用父工程 **Spring AI** 版本。
...
mvn -q compile exec:java -Dexec.mainClass="io.github.partmeai.Main"
```
