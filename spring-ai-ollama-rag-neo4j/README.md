...
> 基于 **Spring AI** [Ollama](https://ollama.com/) 嵌入与对话，以及 **Neo4j** 向量存储的 RAG 示例。版本与 BOM 由父工程 `spring-ai-examples/pom.xml` 统一管理（Spring AI 1.1.x）。
...
- 向量存储 Starter：`spring-ai-starter-vector-store-neo4j`
- 请先启动 **Ollama** 与本模块所需的 **Neo4j** 服务，再修改 `src/main/resources/application.yml` 中的连接与模型名。

### 运行

```bash
cd spring-ai-ollama-rag-neo4j
mvn spring-boot:run
```
